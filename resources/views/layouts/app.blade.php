    ...
    <nav x-data="{ open: false }" class="bg-white border-b border-gray-200 shadow-sm"><!-- changed Navbar -->
        <div class="container mx-auto px-4 py-4 flex justify-between items-center">
            ...
            <div class="hidden md:flex items-center">
                <a href="{{ route('home') }}"
                    class="p-2 text-base font-semibold text-gray-800 hover:text-indigo-600 transition">Home</a>
                <a href="{{ route('reports.index') }}"
                    class="p-2 text-base font-semibold text-gray-800 hover:text-indigo-600 transition">Reports</a>
                <a href="{{ route('report.history') }}"
                    class="p-2 text-base font-semibold text-gray-800 hover:text-indigo-600 transition">Report Logs</a>

                <a href="{{ route('resident.announcements') }}"
                    class="p-2 text-base font-semibold text-gray-800 hover:text-indigo-600 transition">Announcements</a>
                <a href="{{ route('resident.services') }}"
                    class="p-2 text-base font-semibold text-gray-800 hover:text-indigo-600 transition">Services</a>
                <a href="{{ route('resident.about') }}"
                    class="p-2 text-base font-semibold text-gray-800 hover:text-indigo-600 transition">About</a>
                ...
                @auth
                    <div x-data="{ open: false }" class="relative">
                        <button @click="open = !open"
                            class="inline-flex items-center gap-2 border border-gray-300 text-sm text-gray-700 px-3 py-1 rounded-md bg-white shadow-sm hover:bg-gray-50 transition">
                            <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 text-indigo-500" fill="none"
                                viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                <path stroke-linecap="round" stroke-linejoin="round"
                                    d="M5.121 17.804A13.937 13.937 0 0112 15.75c2.175 0 4.212.517 6.038 1.426M15 11.25a3 3 0 11-6 0 3 3 0 016 0z" />
                            </svg>
        ...
        <div x-show="open" class="fixed inset-0 z-50 md:hidden">
            ...
            <div x-show="open" x-transition:enter="transform transition ease-in-out duration-300"
                ...
                <!-- Navigation Links -->
                <a href="{{ route('home') }}" class="block py-2 text-gray-800 font-semibold">Home</a>
                <a href="{{ route('reports.index') }}" class="block py-2 text-gray-800 font-semibold">Reports</a>
                <a href="{{ route('report.history') }}" class="block py-2 text-gray-800 font-semibold">Report
                    Logs</a>
                <a href="{{ route('resident.announcements') }}"
                    class="block py-2 text-gray-800 font-semibold">Announcements</a>
                <a href="{{ route('resident.services') }}"
                    class="block py-2 text-gray-800 font-semibold">Services</a>
                <a href="{{ route('resident.about') }}" class="block py-2 text-gray-800 font-semibold">About</a>
                ...
                <!-- Profile -->
                @auth
                    <div class="mb-2 text-gray-600">Hi, {{ Auth::user()->name }}</div>

                    <a href="{{ route('profile.useredit') }}" class="block py-2 text-gray-800">
                        ⚙️ Edit Profile
                    </a>
                ...
                @else
                    <a href="{{ route('login') }}"
                        class="block bg-green-600 text-white py-2 px-4 mt-2 rounded-md text-center">
                        Login
                    </a>
                @endauth
