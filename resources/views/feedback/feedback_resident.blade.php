    ...
    <div class="max-w-3xl mx-auto p-6 bg-white rounded-xl shadow">
        ...
        <form method="POST" action="{{ route('feedback.store', $report->id) }}" enctype="multipart/form-data">
            ...
            <div class="mb-5">
                ...
                <select name="rating" required @if ($alreadyRated) disabled @endif
                    class="w-full border rounded-md px-3 py-2 focus:ring focus:ring-indigo-200">
                    <option value="">Select rating</option>
                    @for ($i = 1; $i <= 5; $i++)
                        <option value="{{ $i }}" @if (old('rating', $existingFeedback->rating ?? '') == $i) selected @endif>
            ...
            <div class="mb-5">
                ...
                <textarea name="feedback" rows="4" required @if ($alreadyRated) readonly @endif
                    class="w-full border rounded-md px-3 py-2 focus:ring focus:ring-indigo-200"
            ...
            <div class="mb-5">
                ...
                @if (!$alreadyRated)
                    <input type="file" name="photo" id="photoInput" required accept="image/*"
                        onchange="previewPhoto(event)" class="block w-full text-sm text-gray-600">
                ...
                @else
                    @php
                        $photoUrl = reportMediaUrl($existingFeedback->photo);
                    @endphp

                    @if ($photoUrl)
                        <div class="mt-3">
                            <img src="{{ $photoUrl }}" class="max-h-60 rounded border" alt="Feedback Photo">
                        </div>
                    @endif
                @endif
            ...
            <div class="mb-6">
                ...
                @if (!$alreadyRated)
                    <input type="file" name="video" id="videoInput" required accept="video/*"
                        onchange="previewVideo(event)" class="block w-full text-sm text-gray-600">

                    <div id="videoContainer" class="mt-3 hidden">
                        <video id="videoPreview" class="w-full max-h-64 rounded border mb-2" controls>
                        </video>
                ...
                @else
                    @php
                        $videoUrl = reportMediaUrl($existingFeedback->video);
                    @endphp
